...
class UserCreatedListenerTest extends TestCase {
	public function testStaleRestrictionIsClearedForRecreatedUserId(): void {
		...
		$listener = new UserCreatedListener($service, $logger);

		$listener->handle(new UserCreatedEvent($this->createUser('alice'), 'password'));
	}
	...
	public function testCleanupThrowableIsSwallowedAndLogged(): void {
		...
		$logger->expects(self::once())
			->method('error')
			->with(
				'Could not clear stale User Lockdown data for a new user.',
